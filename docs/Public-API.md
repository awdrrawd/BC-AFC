...
`getLoverRoom()` 保持同步讀取：非 AFC 戀人回傳 null；依序使用同房間資料、在線好友公開房名、私人房分享紀錄。公開房不寫入私人房快取，查詢本身不發送請求。進房、房間設定更新及離房都會通知在線戀人。

`getLovers()` 維持原有陣列格式與排序，回傳深複本，修改回傳物件不會更動 AFC 設定。

登入後 5 秒開始整批檢查戀人房間狀態，最多 5 輪；每輪完成後隔 5 秒補查尚未確認的人。每輪重新取得在線好友名單：離線視為已確認並清除舊房名；公開房採用伺服器房名；其餘在線戀人透過雙向請求確認房間或大廳。缺少房名不等於大廳，必須收到房名或明確清除通知。全部確認後停止，未回覆者到達上限也停止。請求同時分享自己的房間，回覆不觸發另一輪請求。沒有個別查詢指令或公開請求 API。
