...
import { readLoverBackup, writeLoverBackup } from './lover-backup.js';
...
function syncBackup() {
    writeLoverBackup(getSharedSettings()?.lovers ?? []);
}
...
export function reconcileLocalDB() {
    ...
    if (!dbHas &&  ossHas) { syncBackup(); return; }        // 換裝置：本機天生空 → 靜默回填，不警告
    ...
    if (_loverSetEqual(oss, db)) { syncBackup(); return; }  // 一致：更新時間戳即可
